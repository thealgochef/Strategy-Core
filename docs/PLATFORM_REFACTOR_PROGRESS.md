...
- **Current:** W3 (PROVE; split **W3a** bundle-build / **W3b** parity gate + falsification) — **W2 COMPLETE and PUSHED** (pins held `256020c` — no consumer-facing SC change this window; CI green on all three repos incl. SC's first `ci.yml` run; W2-FIX SC `32e9cc0`+`ecbc15e`, TL `df6b2c4` + lint `5a8d28a` recorded). See POST-E3 section. S9.9/F parked.
...
### Status
Phases A–E3: COMPLETE. W1: COMPLETE (pushed; pins bumped; CI witnessed). W2: COMPLETE (pushed; pins held `256020c` — no consumer-facing SC change; CI ×3 green incl. SC's first run; W2-FIX + the TL lint commit `5a8d28a` recorded). Current: W3 — **W3a executed 2026-06-12 (vectorization + fold/pin plumbing landed; the P2 stop-gate's reader cost RESOLVED by W3A-READER under D-P-15: decode vectorized, identity-proven on 2 full days, drift net green, row-wise path deleted; build projection 24.1 h → ~8.7 h. BUILD LAUNCH PENDING the review go — command verbatim in the W3A-READER subsection)**; W3b (parity gate + falsification) follows the bundle. S9.9, F1–F3: PARKED behind W3 green + soak. Live model outputs remain DECORATIVE until W3's gate is green on the fresh bundle.
