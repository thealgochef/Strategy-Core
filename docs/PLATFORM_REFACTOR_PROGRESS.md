...
- **Current:** W3 (PROVE; split **W3a** bundle-build / **W3b** parity gate + falsification) — **W3a bundle BUILT on D-036 (`NQ_W3_20260613T055600Z`); W3b gate run (73-day) = 236/236 BIT-EXACT, one RED 2026-02-12 → root-caused to a None-seeded standalone-build cache the warmer skipped and RESOLVED (5-touch regen + seed guard QL `098e354`); `098e354` + 6 preceding W3a commits greenlit, push pending.** CAVEAT: build + gate ran on the STALE 583-line site-packages reader (bit-exact green = consistency, NOT correctness); the vectorized reader SC `37359ae` is now validated CORRECT vs raw ground truth (`READER_CORRECTNESS_PROOF.md`, Trade-Lab root). **IN FLIGHT: vectorized-reader redeploy + W3 bundle rebuild + W3b re-gate on it — LOCAL, push pending.** (W2 COMPLETE and PUSHED; pins held `256020c`; CI ×3 green; W2-FIX SC `32e9cc0`+`ecbc15e`, TL `df6b2c4` + lint `5a8d28a` recorded.) See POST-E3 §W3b. S9.9/F parked.
...
**IN FLIGHT:** redeploy the vectorized reader (SC `37359ae`) into both consumers, **rebuild the W3 bundle on it**, and **re-run the W3b gate** against that rebuild — so train + serve + gate all sit on the validated reader. LOCAL; push pending.
