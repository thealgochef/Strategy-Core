...
### Status
Phases A–E3: COMPLETE. W1: COMPLETE (pushed; pins bumped; CI witnessed). W2: COMPLETE (pushed; pins held `256020c` — no consumer-facing SC change; CI ×3 green incl. SC's first run; W2-FIX + the TL lint commit `5a8d28a` recorded). Current: W3 — **W3a executed 2026-06-12 (vectorization + fold/pin plumbing landed; TRAIN STOP-GATED at 24.1 h projected vs the 90-min gate — reader-cost ruling pending)**; W3b (parity gate + falsification) follows the ruling + the fresh bundle. S9.9, F1–F3: PARKED behind W3 green + soak. Live model outputs remain DECORATIVE until W3's gate is green on the fresh bundle.
